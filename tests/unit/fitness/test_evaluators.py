...
class TestPrometheusEvaluator:
    def test_evaluate_point_success(self):
        ...
        evaluator = PrometheusEvaluator(
            mock_prom, "my_query", FitnessFunctionType.point, retries=1
        )
    ...
    def test_evaluate_range_success(self):
        mock_prom = Mock()
        mock_prom.process_prom_query_in_range.return_value = [
            {"values": [[1000, "50"]]}
        ]

        evaluator = PrometheusEvaluator(
            mock_prom, "my_query", FitnessFunctionType.range, retries=1
        )
        score = evaluator.evaluate(datetime.now(), datetime.now())
        assert score == 50.0
...
class TestHealthCheckEvaluator:
    def test_success_rate_mode(self):
        ...
        context = {
            "health_check_results": {
                "url1": [
                    HealthCheckResult(
                        name="t1", status_code=200, success=True, response_time=0.1
                    ),
                    HealthCheckResult(
                        name="t1", status_code=500, success=False, response_time=0.1
                    ),
                ]
            }
        }
    ...
    def test_response_time_mode(self):
        ...
        results = [
            HealthCheckResult(name="t", status_code=200, success=True, response_time=t)
            for t in times
        ]
...
class TestWeightedAggregator:
    def test_aggregation(self):
        ...
        aggregator = WeightedAggregator([(eval1, 0.6), (eval2, 0.4)])
