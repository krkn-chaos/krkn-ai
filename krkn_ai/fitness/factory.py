...
class RetryEvaluator(BaseFitnessEvaluator):
    def __init__(self, base_evaluator: BaseFitnessEvaluator, retries: int = 3, retry_delay: float = 10.0):
    ...
    def calculate(self, result: CommandRunResult) -> float:
        ...
        raise FitnessFunctionCalculationError(f"Fitness failed after {self.retries} retries")


class FitnessEvaluatorFactory:
    @staticmethod
    def create_evaluator(config: EvaluatorConfig, context: Dict[str, Any]) -> BaseFitnessEvaluator:
        """
