...
from krkn_ai.fitness.factory import FitnessEvaluatorFactory
...
def test_fitness_evaluator_factory_creation():
    ...
    prom_config = EvaluatorConfig(
        name="prometheus", weight=1.5, properties={"query": "up", "type": "point"}
    )
    ...
    hc_config = EvaluatorConfig(
        name="health_check", weight=2.0, properties={"metric": "outliers"}
    )
...
def test_weighted_aggregator():
    ...
    eval_configs = [
        ...
        EvaluatorConfig(
            name="health_check", weight=0.4, properties={"id": 2, "metric": "outliers"}
        ),
    ]
    ...
    health_results = {
        "http://app1": [
            ...
            HealthCheckResult(
                name="app1", status_code=200, success=True, response_time=0.13
            ),
            HealthCheckResult(
                name="app1",
                status_code=500,
